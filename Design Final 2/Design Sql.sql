...
BEGIN
RETURN QUERY
Select C.name, L.LocationName, T.TimeName1
From Characters c inner join ConfidentLocations CL on C.cid = CL.cid,
	 Locations L inner join ConfidentLocations where L.Lid = CL.Lid,
     Time T inner join ConfidentLocations where T.Tid = CL.Tid
...
RETURN QUERY
Select I.ItemName
From Item I inner join HelpfulItems HI on I.Itemid = HI.Itemid
	Where I.effects = item;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION HealAll(Heal) RETURNS TRIGGER AS $$
DECLARE
   Heal  int;
Begin
	If (Playable.HP < 500)
